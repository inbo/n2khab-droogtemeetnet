...
**het _gemiddelde_ van de _jaarlijkse_ 5%-percentielen van de grondwatermetingen voor het meetpunt voor de periode 1985 - 2014**, cfr. \@ref(h3:drempelwaarde), die periodiek, bv. elke vijftien jaar, met meer recente data kan herijkt worden, cfr. \@ref(h3:referentietoestand).
...
plotfitting <- function(indic_basis, respons, gemid, og, bg, yas) {
  og <- enquo(og)
  bg <- enquo(bg)
    ...
    ungroup() %>% 
    inner_join(indic_basis)
  
  p <- ggplot(data = PI_data, aes_string(y = respons, x = "jaar")) +
    xlab("Jaar") + ylab(yas) +
    geom_point(shape = 16, size = 1, col = inbo_lichtblauw) +
    geom_line(aes_string(x = "jaar", y = gemid), size = 1, color = inbo_rood) +
...
plotma.basis <- function(indic_finaal, indicator, intercept) {
  p <- ggplot(data = indic_finaal ) + 
    geom_line(data = indic_finaal %>% filter(jaar >= 1985 + tijdvenster), aes(x = jaar, y = lcl), color = inbo_rood, linetype = "longdash", alpha = 0.50) +
    geom_line(data = indic_finaal %>% filter(jaar <= 1985 + tijdvenster), aes(x = jaar, y = lcl), color = inbo_rood, linetype = "longdash", alpha = 0.10) +
    geom_ribbon(data = indic_finaal %>% filter(jaar >= 1985 + tijdvenster), aes(x = jaar, ymax = ucl, ymin = lcl),
                fill = "lightpink",
                alpha = 0.4) +
    geom_ribbon(data = indic_finaal %>% filter(jaar <= 1985 + tijdvenster), aes(x = jaar, ymax = ucl, ymin = lcl),
                fill = "lightpink",
                alpha = 0.15) +
    geom_line(data = indic_finaal %>% filter(jaar >= 1985 + tijdvenster), aes(x = jaar, y = ucl), color = inbo_rood, linetype = "longdash", alpha = 0.50) +
    geom_line(data = indic_finaal %>% filter(jaar <= 1985 + tijdvenster), aes(x = jaar, y = ucl), color = inbo_rood, linetype = "longdash", alpha = 0.10) +
    geom_line(data = indic_finaal %>% filter(jaar >= 1985 + tijdvenster), aes_string(x = "jaar", y = indicator), color = inbo_rood ) +
    geom_line(data = indic_finaal %>% filter(jaar <= 1985 + tijdvenster), aes_string(x = "jaar", y = indicator), color = inbo_rood, alpha = 0.15 ) +    
    geom_hline(aes(yintercept = intercept), color = inbo_oranje, linetype = "dotted")
  return(p)
}
...
plotma.basis_1venster <- function(indic_finaal, indicator, intercept, tijdvenster) {
  ...
  p <- ggplot(data = indic_finaal_1 ) + 
    geom_line(data = indic_finaal_1 %>% filter(jaar >= 1985 + tv), aes(x = jaar, y = lcl), color = inbo_rood, linetype = "longdash", alpha = 0.50) +
    geom_line(data = indic_finaal_1 %>% filter(jaar <= 1985 + tv), aes(x = jaar, y = lcl), color = inbo_rood, linetype = "longdash", alpha = 0.10) +
    ...
    geom_ribbon(data = indic_finaal_1 %>% filter(jaar <= 1985 + tv), aes(x = jaar, ymax = ucl, ymin = lcl),
                fill = "lightpink",
                alpha = 0.15) +
    geom_line(data = indic_finaal_1 %>% filter(jaar >= 1985 + tv), aes(x = jaar, y = ucl), color = inbo_rood, linetype = "longdash", alpha = 0.50) +
    geom_line(data = indic_finaal_1 %>% filter(jaar <= 1985 + tv), aes(x = jaar, y = ucl), color = inbo_rood, linetype = "longdash", alpha = 0.10) +
    geom_line(data = indic_finaal_1 %>% filter(jaar >= 1985 + tv), aes_string(x = "jaar", y = indicator), color = inbo_rood ) +
    geom_line(data = indic_finaal_1 %>% filter(jaar <= 1985 + tv), aes_string(x = "jaar", y = indicator), color = inbo_rood, alpha = 0.15 ) +    
    geom_hline(aes(yintercept = intercept), color = inbo_oranje, linetype = "dotted")
  return(p)
  }
...
plottrend <- function(indic_finaal, indicator, intercept, yrechts) {
    p <- plotma.basis(indic_finaal, indicator, intercept)
    p <- p +  
      geom_line(data = indic_finaal %>% filter(jaar >= 1985 + tijdvenster), aes(x = jaar, y = og_jaar), color = inbo_steun_blauw, linetype = "longdash", alpha = 0.5) +
      geom_line(data = indic_finaal %>% filter(jaar <= 1985 + tijdvenster), aes(x = jaar, y = og_jaar), color = inbo_steun_blauw, linetype = "longdash", alpha = 0.10) +
      geom_ribbon(data = indic_finaal %>% filter(jaar >= 1985 + tijdvenster), aes( x = jaar, ymax = bg_jaar, ymin = og_jaar),
                  fill = grey(0.5),
                  alpha = 0.4) +  
      geom_ribbon(data = indic_finaal %>% filter(jaar <= 1985 + tijdvenster), aes( x = jaar, ymax = bg_jaar, ymin = og_jaar),
                  fill = grey(0.5),
                  alpha = 0.15) +  
      geom_line(data = indic_finaal %>% filter(jaar >= 1985 + tijdvenster), aes(x = jaar, y = bg_jaar), color = inbo_steun_blauw, linetype = "longdash", alpha = 0.5) +
      geom_line(data = indic_finaal %>% filter(jaar <= 1985 + tijdvenster), aes(x = jaar, y = bg_jaar), color = inbo_steun_blauw, linetype = "longdash", alpha = 0.10) +
      geom_line(data = indic_finaal %>% filter(jaar >= 1985 + tijdvenster), aes(x = jaar, y = gem_jaar), color = inbo_steun_blauw) +
      geom_line(data = indic_finaal %>% filter(jaar <= 1985 + tijdvenster), aes(x = jaar, y = gem_jaar), color = inbo_steun_blauw, alpha = 0.15) +    
      geom_hline(aes(yintercept = intercept), color = inbo_oranje, linetype = "dotted") +
      scale_x_continuous(breaks = 1985:2020, labels = insert_minor(seq(1985, 2020, by = 5), 4)) +
      labs(x = "Jaar", y = "trend") +
      scale_y_continuous(sec.axis = dup_axis(name = yrechts)) +
      facet_wrap(vars('tijdvenster (jaren) ' = tijdvenster), labeller = label_both) +
      theme( axis.text.y.right = element_text(color = inbo_rood, size = 14), 
        axis.ticks.y.right = element_line(color = inbo_rood), 
        axis.title.y.left =  element_text(color = inbo_grijsblauw, size = 14),
        axis.title.y.right =  element_text(color = inbo_rood, size = 14),
...
plottrend.1venster <- function(indic_finaal, indicator, intercept, tijdvenster, yrechts) {
    p <- plotma.basis_1venster(indic_finaal, indicator, intercept, tijdvenster)
    tv <- tijdvenster
    indic_finaal_1 <- indic_finaal %>% filter(tijdvenster == tv)    
    p <- p +  
      geom_line(data = indic_finaal_1 %>% filter(jaar >= 1985 + tv), aes(x = jaar, y = og_jaar), color = inbo_steun_blauw, linetype = "longdash", alpha = 0.5) +
      geom_line(data = indic_finaal_1 %>% filter(jaar <= 1985 + tv), aes(x = jaar, y = og_jaar), color = inbo_steun_blauw, linetype = "longdash", alpha = 0.10) +
      ...
      geom_ribbon(data = indic_finaal_1 %>% filter(jaar <= 1985 + tv), aes( x = jaar, ymax = bg_jaar, ymin = og_jaar),
                  fill = grey(0.5),
                  alpha = 0.15) +  
      geom_line(data = indic_finaal_1 %>% filter(jaar >= 1985 + tv), aes(x = jaar, y = bg_jaar), color = inbo_steun_blauw, linetype = "longdash", alpha = 0.5) +
      geom_line(data = indic_finaal_1 %>% filter(jaar <= 1985 + tv), aes(x = jaar, y = bg_jaar), color = inbo_steun_blauw, linetype = "longdash", alpha = 0.10) +
      geom_line(data = indic_finaal_1 %>% filter(jaar >= 1985 + tv), aes(x = jaar, y = gem_jaar), color = inbo_steun_blauw) +
      geom_line(data = indic_finaal_1 %>% filter(jaar <= 1985 + tv), aes(x = jaar, y = gem_jaar), color = inbo_steun_blauw, alpha = 0.15) +    
      geom_hline(aes(yintercept = intercept), color = inbo_oranje, linetype = "dotted") +
      scale_x_continuous(breaks = 1985:2020, labels = insert_minor(seq(1985, 2020, by = 5), 4)) +
      labs(x = "Jaar", y = "trend") +
      scale_y_continuous(sec.axis = dup_axis(name = yrechts)) +
...
# aanvulling/correctie 2024

tijdreeksen_vernieuwd <- read_csv(file.path("data", "interim", "tijdreeksen_vernieuwd.csv"))

ruwetabellen_lijst <- data.frame (a = "droogte24_tijdreeks", b = "ruwedata24_", 
                                  c = seq(from = 1, to = 8))
...
for (i in seq(from = 1, to = 8)){
  write_vc(get(sprintf("ruwedata24_%02d", i)), file.path("data", "local", sprintf("ruwedata%02d", i)))  
}
```

```{r wegschrijven-brontabellen-naar-rds-formaat, eval=FALSE }
for (i in seq(from = 1, to = 23)){
  saveRDS(get(sprintf("ruwedata%02d", i)), file.path("data", "local", paste0(sprintf("ruwedata%02d", i), ".rds")))
}

for (i in seq(from = 1, to = 8)){
  saveRDS(get(sprintf("ruwedata24_%02d", i)), 
            file.path("data", "local", paste0(sprintf("ruwedata24_%02d", i), ".rds")))
}
```

```{r inlezen-vc-tabellen, include=FALSE, eval=!file.exists(file.path("data", "local", "ruwedata_2024.rds" ))}
...
```{r inlezen-vc-tabellen-uit-cache, include=FALSE, eval=!file.exists(file.path("data", "local", "ruwedata_2024.rds" ))}
ruwetabellen_lijst <- data.frame(bron = "ruwedata", doel = "ruwedata", c = seq(from = 1, to = 23))
...
```{r inlezen-rds-tabellen-uit-cache, include=FALSE, eval=!file.exists(file.path("data", "local", "ruwedata_2024.rds" ))}
check <- readRDS(file.path("cache", paste0("ruwedata", sprintf("%02d", 1), ".rds")))
 all.equal(check %>% 
             arrange(meetpunt_import, dag) %>% data.frame(), ruwedata01 %>% 
             arrange(meetpunt_import, dag) %>% data.frame())
 
check2 <- readRDS(file.path("data", "local", paste0("ruwedata", sprintf("%02d", 1), ".rds")))
 all.equal(check %>% 
             arrange(meetpunt_import, dag) %>% data.frame(), check2 %>% 
             arrange(meetpunt_import, dag) %>% data.frame())
 
# basistabellen (versie 2020)
ruwetabellen_lijst <- data.frame(bron = "ruwedata", doel = "ruwedata", c = seq(from = 1, to = 23))

ruwetabellen_lijst_rds <- ruwetabellen_lijst %>% 
  mutate(naambrontabel = file.path("data", "local", paste0(bron,sprintf("%02d", c), ".rds")),
         naamdoeltabel = sprintf(paste0(doel,"%02d"), c)) 
ruwetabellen_lijst_rds <- setNames(ruwetabellen_lijst_rds %>% 
                                     dplyr::pull(naambrontabel), 
                                   make.names(ruwetabellen_lijst_rds %>% 
                                                dplyr::pull(naamdoeltabel)))

list2env(lapply(X = ruwetabellen_lijst_rds, FUN = readRDS), 
  envir = .GlobalEnv)

# aanvulling 2024
ruwetabellen_lijst <- data.frame(bron = "ruwedata24_", doel = "ruwedata24_", c = seq(from = 1, to = 8))
...
```{r samenvoegen-bestanden, include=FALSE, eval=!file.exists(file.path("data", "local", "mruwedata_fin.rds" ))}
...
if (!file.exists(file.path("data", "local", "ruwedata_2020.rds"))) {
  ruwedata_2020 <- purrr::map_dfr(sprintf("ruwedata%02d",1:23), get )
  saveRDS(ruwedata_2020, file.path("data", "local", "ruwedata_2020.rds"))
  rm(list = ls(pattern = "^ruwedata20"))  
} else {
  ruwedata_2020 <- readRDS(file.path("data", "local", "ruwedata_2020.rds"))
}
if (!file.exists(file.path("data", "local", "ruwedata_2024.rds"))) {
  ruwedata_2024 <- purrr::map_dfr(sprintf("ruwedata24_%02d",1:8), get )
  saveRDS(ruwedata_2024, file.path("data", "local", "ruwedata_2024.rds"))
  rm(list = ls(pattern = "^ruwedata24"))  
} else {
  ruwedata_2024 <- readRDS(file.path("data", "local", "ruwedata_2024.rds"))
}

#samenvoegen bestanden
ruwedata <- ruwedata_2020 %>% 
  mutate(ronde = 1) %>% 
  bind_rows(ruwedata_2024 %>% 
              mutate(ronde = 2))
# veleptn <- c("COOP001", "DYLP029", "HBTP007", "IJSP023",
#                          "KALP022", "KBRP056", "LDOP041", "MOLP023",
#                          "WALP118")
# ruwedata20.veleptn <- ruwedata_2020 %>%
#   filter(meetpunt %in% veleptn)
# ruwedata24.veleptn <- bind_rows(ruwedata24_01, ruwedata24_02, ruwedata24_03,
#                                 ruwedata24_04, ruwedata24_07) %>%
#   filter(meetpunt %in% veleptn)
# 
# ruwedata.veleptn <- ruwedata20.veleptn %>%
#   mutate(ronde = 1) %>%
#   bind_rows(ruwedata24.veleptn %>%
#               mutate(ronde = 2))

# er werden veldmetingen toegevoegd buiten de periode waarvoor simulaties voor berekend werden. Deze mogen gerust verwijderd worden. Het simulatienr is hier leeg.
ruwedata <- ruwedata %>% 
  filter(!is.na(simulatienr))
# ruwedata.veleptn <- ruwedata.veleptn %>%
#   filter(!is.na(simulatienr))

# #dubbels (er zijn er zeker)
# # duurt heel lang !
# check <- ruwedata %>% 
#   filter(meetpunt_import != meetpunt) %>% 
#   count(meetpunt, simulatienr, dag) %>% 
#   filter(n > 1)
# 
# test <- ruwedata %>% 
#   filter(meetpunt == "ASBP003")

saveRDS(ruwedata, file.path("data", "local", "ruwedata_start.rds"))
# ruwedata <- ruwedata %>% filter(!meetpunt %in% veleptn) %>%
#   bind_rows(ruwedata.veleptn)

# volgende berekening duurt een paar uren !
ruwedata <- ruwedata %>% 
  group_by(meetpunt, simulatienr, dag) %>% 
  slice_max(order_by = ronde, with_ties = TRUE) %>% # we geven voorrang aan de laatste versie, want dit is de verbeterde versie
  ungroup()

# check <- ruwedata.veleptn %>%
#   group_by(meetpunt, simulatienr, dag) %>%
#   slice_max(order_by = ronde, with_ties = TRUE) %>% # we geven voorrang aan de laatste versie, want dit is de verbeterde versie
#   ungroup()
# # 
# ruwedata.rds <- readRDS(file.path("data", "local", "ruwedata_vervolg.rds"))
# ruwedata.vervolg <- ruwedata.rds %>% filter(!meetpunt %in% veleptn) %>%
#   bind_rows(check)

saveRDS(ruwedata, file.path("data", "local", "ruwedata_vervolg.rds"))


# er zitten nog dubbels in, deze moeten blijven ! Op dagen met een veldmeting is er ook een simulatiewaarde (ook voor simulatienr =1). Alle simulatiewaarden (en niet alleen voor simulatienr = 1) moeten verwijderd worden, want ze kunnen afwijken van de veldmeting, die we uiteraard prioriteit willen geven.

# #check op dubbels duurt ook enkele minuten
# check.dubbels <- ruwedata %>% count(meetpunt, simulatienr, dag) %>% 
#   filter(n > 1)


# verwijderen van simulaties op dagen dat er een veldmeting is + 7 dagen ervoor en 7 dagen erna
# is_veldmeting = 1 => veldmeting
# is_veldmeting = -1 => simulatie op minder dan 7 dagen van een veldmeting: dus te schrappen
# is_veldmeting = 0 => simulatiemeting, te behouden
mruwedata <- ruwedata %>% 
  filter((meetpunt_import == meetreeks & is_veldmeting == 1) | is_veldmeting == 0) 

saveRDS(mruwedata, file.path("data", "local", "mruwedata.rds"))
...
check4 <- checkmruw %>% 
  inner_join(check3 %>% filter(n == 1), by = join_by(dag))



mruwedata.veld <- mruwedata %>% 
  filter(is_veldmeting == 1) 
check.dubbels <- mruwedata.veld %>% count(meetpunt, simulatienr, dag) %>%
  filter(n > 1)
# = 0 => ok
saveRDS(mruwedata.veld, file.path("data", "local", "mruwedata_veld.rds"))

# deze veldmetingen moeten nog gemultipliceerd worden tot 20, omdat er 20 simulaties berekend werden (met dus voor die dagen constante waarden)
simreeksnr.add <-  c(2:20)
# test <- mruwedata.veld %>% filter(meetpunt == "ASBP003")
# test.basis <- mruwedata %>% filter(meetpunt == "ASBP003")
mruwedata.add <- map(simreeksnr.add, .f = function(x){
  reeks.add <- mruwedata.veld %>% 
    mutate(simulatienr = x)
}) %>% list_rbind()

# samenvoegen, duurt ook lang
mruwedata <- mruwedata %>% 
  bind_rows(mruwedata.add)
saveRDS(mruwedata, file.path("data", "local", "mruwedata_vervolg.rds"))
# check wat het doet om onvolledige meetreeksen (<365 metingen/jaar) te hebben, dit is nl. nu het geval als er periodieke (minder frequent dan dagelijks) veldmetingen zijn, want gesimuleerde metingen in een tijdsinterval van 7 dagen voor en na zijn geschrapt, omdat deze teveel de veldmetingen zouden overrulen.
reeks.vol <- data.frame(dag = 1:365, meting_TAW = seq_len(365))
reeks.periodiek <- data.frame(dag = seq(from = 1, to = 365, by = 7), meting_TAW = seq(from = 1, to = 365, by = 7))

absperc.vol <- reeks.vol %>% 
...
absperc.periodiek <- reeks.periodiek %>% 
  summarise(p01 = quantile(meting_TAW, 1/100),
            p05 = quantile(meting_TAW, 5/100),
            p10 = quantile(meting_TAW, 10/100),
            p30 = quantile(meting_TAW, 30/100),
...
all.equal(absperc.vol, absperc.periodiek)
# heel goed nieuws !, het ontbreken van metingen heeft geen directe impact op de percentielberekening 


#data cleaning - algemeen
# 1) beperken van de meetreeks in lengte: van 1985 tot en met 2022 (2023 was onvolledig)
# 2) factoriseren van meetpunt en weglaten van ongebruikte velden 

mruwedata.fin <-  mruwedata %>% 
  filter(between(year(dag),1985, 2022)) %>%  
  mutate(meetpunt = factor(meetpunt)) %>% 
  dplyr::select(-meetpunt_import, -meetreeks, -is_veldmeting, -ronde) #om de grootte van het databestand te reduceren

# bewaren
saveRDS(mruwedata.fin, file.path("data", "local", "mruwedata_fin.rds"))

# opruimen
rm(list = ls(pattern = "^ruwedata|^ruwetabellen|^tijdreeksen_v|^reeks|\\.add$|\\.veld|^test|^absperc|^check|mruwedata.add|mruwedata.veld"))
gc()
```
...
```{r verwijderen-ruwe-tabellen, eval = TRUE}
...
rm(list = sprintf("ruwedata%02d", c(2:18,20:23)))
rm(list = sprintf("ruwedata24_%02d", c(1:3,5:7)))
```

```{r tubes-network, include=FALSE, eval=TRUE}
...
```{r basisdata-final, include=FALSE, eval= FALSE}
# eval=!file.exists(file.path("cache", "basisdata.rds" ))
...
```{r basisdata-final-uit-cache, include=FALSE, eval=!file.exists(file.path("data", "local", "mruwedata_fin.rds" ))}
mruwedata.fin <- readRDS(file.path("data", "local", "mruwedata_fin.rds" ))
```


### Berekenen van de kritische drempelwaarden en de absolute indicatorwaarden {#h3:calcdrempelwaarde}
Een absolute indicator vergelijkt dagelijks het grondwaterpeil met een kritische drempelwaarde. 
De dagen dat het grondwaterpeil onder deze drempelwaarde valt, worden per jaar opgeteld (\@ref(h2:methode-drempelwaarde) en \@ref(h2:methode-indicator)).
Dit zijn de absolute indicatorwaarden.

```{r absolute-drempelwaarden, include=FALSE, eval = !file.exists(file.path("data", "local","percentielen.rds" ))}
absperc <- mruwedata.fin %>% 
...
```{r gwgroups, include=FALSE, eval = !file.exists(file.path("data", "local", "percentielen.rds" ))}
...
saveRDS(absperc_gw,file.path("data", "local", "percentielen.rds" ))

write_vc(absperc_gw, file.path("data", "result", "percentielen-1985-2014"), root = projroot, sorting = c("meetpunt"))
# absperc_gw_oud <- read_vc( file.path("data", "result", "percentielen-1985-2014"), root = projroot)
```

```{r gwgroups-uit-cache, include=FALSE}
absperc_gw <- readRDS(file.path("data", "local", "percentielen.rds" ))
```

```{r abs-indicator, eval = !file.exists(file.path("data","local", "dagendroognat.rds" ))}

dagen_droog_nat <- mruwedata.fin %>% #oude objectnaam: indic_abs_basis
  ...
  mutate( 
          ...
          dag_boven_p99 = if_else(meting_TAW > p99,1,0),
          dag_boven_p95 = if_else(meting_TAW > p95,1,0),
          dag_boven_p90 = if_else(meting_TAW > p90,1,0),
          dag_boven_p70 = if_else(meting_TAW > p70,1,0)         
  ) %>% 
  group_by(meetpunt, simulatienr,jaar, aantaldagen_jaar, groep3, groep3n) %>% 
  summarise_at(vars(dag_onder_p01:dag_boven_p70), sum) %>% 
  ungroup() %>% 
...
```
```{r rollend-gemiddelde, eval = !file.exists(file.path("data","local", "dagendroognat.rds" ))}
#berekenen van rollende gemiddelden

modeldata <- dagen_droog_nat %>% 
  group_by(meetpunt, simulatienr) %>% 
  arrange(jaar) %>% 
  reframe(dag_onder_p05.0 = dag_onder_p05,
            ...
            dag_boven_p95.0 = dag_boven_p95,
            dag_boven_p95.2 = movavg(dag_boven_p95, n = 2, "w"), 
            dag_boven_p95.4 = movavg(dag_boven_p95, n = 4, "w"),
            dag_boven_p95.6 = movavg(dag_boven_p95, n = 6, "w"),
            dag_boven_p95.8 = movavg(dag_boven_p95, n = 8, "w"),
            dag_boven_p95.10 = movavg(dag_boven_p95, n = 10, "w"),
            dag_boven_p95.20 = movavg(dag_boven_p95, n = 20, "w"),
            dag_boven_p95.30 = movavg(dag_boven_p95, n = 30, "w"),            
...
dagen_droog_nat <- dagen_droog_nat %>% 
  inner_join(modeldata)

#wegschrijven in een RDS-bestand
saveRDS(dagen_droog_nat,file.path("data", "local", "dagendroognat.rds" ))
saveRDS(modeldata,file.path("data", "local", "modeldata.rds" ))
```

```{r abs-indicator-uit-cache}
dagen_droog_nat <- readRDS(file.path("cache", "dagendroognat.rds" ))
...
checkdistributie <- dagen_droog_nat %>% 
...
car::qqPlot(checkdistributie, "pois", lambda = poisson$estimate, id = FALSE, xlab = "Theoretisch (o.b.v. kwantiel) verwachte waarde voor een poisson-verdeling ", ylab = "aantal droge dagen")
...
qqPlot(checkdistributie, "nbinom", size = nbinom$estimate[[1]], mu = nbinom$estimate[[2]], id = FALSE, xlab = "Theoretisch (o.b.v. kwantiel) verwachte waarde voor een negatief binomiale verdeling", ylab = "Aantal droge dagen")
...
#summary(binom)
  ...
  qqp(checkdistributie, "binom", size = 365, prob = binom$estimate, id = FALSE, xlab = "Theoretisch (o.b.v. kwantiel) verwachte waarde voor een binomiale verdeling", ylab = "Aantal droge dagen")
...
De droogte-indicator is op zich vrij eenvoudig te berekenen door voor elk jaar het gemiddelde te berekenen van het aantal droge dagen $d_{mjs}$-waarden, cfr. \@ref(h2:methode-indicator).
...
```{r abs-indic-model-without-trend-function}

indic_abs_function_without_trend <- function(modeldata, suffix= "", respons, percentile, standardised, cache) { 
  #bij deze berekening bekijken we elk jaar afzonderlijk. We beschouwen (nog) geen relatie tussen de jaren. Dit gaan we wel doen bij het onderzoek naar een trend.
  #We kunnen dit bekijken als een mixed-model met vaste effecten obv jaar en random effect obv meetpunt
  ...
  for (ma in tijdvenster) {
    ...
    model_free <- as.formula(paste(respons.ma, "~", "jaar_factor + f(meetpunt, model = 'iid')", sep = " "))    
    ...
    resultname_fitted <- paste0("indic_abs_p", percentile,".", ma, suffix, "_fitted", if (standardised == TRUE) ("_std"))
      ...
      print(paste("tijdvenster = ", ma, " respons = ", respons))    
      #samenstellen van een tweeledig dataframe. De eerste 34 + 58 rijen (#jaren en #meetlocaties) zijn leeg en dienen om de gemiddelde waarden en de bijhorende fouten te berekenen. Het tweede deel dient om het model mee te bouwen    
      mdata <- modeldata
      jaren <- mdata %>% distinct(jaar)
      meetpunten <- mdata %>% distinct(meetpunt)
      ...
      #linear_predictor : deze is niet teruggetransformeerd. We zijn hier alleen geïnteresseerd in de rijen met lege responsen: hier krijg je dan per jaar een gemiddelde predictie én de se.
      #omdat jaar hier als een fixed-effect wordt beschouwd, kan je de predicties halen uit $summary.fixed. De eerste record is wel het  intercept (eerste jaar) en de volgende waarden zijn afwijkingen tov dat intercept. 
      #een gemakkelijker alternatief is ze ook te laten berekenen via linear.predictor
      intercept <- I1$summary.fixed[1,1]
      # result_stat_i <- I1$summary.fixed %>% 
      #   mutate(gemiddelde = ifelse(str_detect(pattern = "jaar", string = row.names(I1$summary.fixed[1])), mean + intercept, mean)) %>% 
      #   bind_cols(jaren) %>% 
      #   dplyr::select(-mode, -kld)    
      # 
      # result_stat_meetpunt_i <- I1$summary.linear.predictor %>% 
      #   slice(1:nrow(meetpunten)) %>% 
      ...
      if (standardised == TRUE) {
        ...
        resultlijst <- list( result_stat, 
                             ...
                             result_fitted)
        names(resultlijst) <- c(resultname_stat, resultname_stat_meetpunt, resultname_fitted)
        list2env(resultlijst, envir = .GlobalEnv)
        
        if (varname_mean %in% colnames(dagen_droog_nat)) {
          dagen_droog_nat <- dagen_droog_nat %>% 
    ...
    tabellen_lijst_rds <- list( 
            naamdoeltabel = file.path(cache, paste0(c(resultname_stat, resultname_stat_meetpunt, resultname_fitted), ".rds")),
            naambrontabel =  c(resultname_stat, resultname_stat_meetpunt, resultname_fitted)) 
    ...
    saveRDS(c(resultname_stat, resultname_stat_meetpunt, resultname_fitted), 
            file.path(cache, paste0("model_namen_intermediair_geentrend.", ma, "_", respons,".rds" )))  
  }
  saveRDS(dagen_droog_nat, file.path(cache, paste0("modelresultaat_geentrend.rds" )))

  return(dagen_droog_nat)
}
```

```{r model-parameters-droog, include=FALSE}
#keuze van parameters
percentile <- "05"
# indicatorname <- paste0("dag_onder_p",percentile)
# indicatorname_inv <- paste0("dag_boven_p",percentile)
standardised <- FALSE
respons.f <- function(indicatorname, percentile, standardised = FALSE){paste0(indicatorname, percentile, if (standardised == TRUE) ("_std"))}

respons <- respons.f("dag_onder_p", percentile = "05")
workflow <- "rekenen"
```

```{r abs-indic-model-without-trend-droog, eval = !file.exists(file.path("data","local", paste0("modelresultaat_geentrend.rds"))) | workflow == "rekenen"}
...
modelkeuze <- data.frame(percentiel = c("01", "05", "10", "30", "95" ), model = c("zeroinflatednbinomial2", "zeroinflatednbinomial0", "zeroinflatednbinomial0", "nbinomial", "zeroinflatednbinomial0"))
modelkeuze$model <- as.character(modelkeuze$model)

dagen_droog_nat <- indic_abs_function_without_trend(modeldata = modeldata, respons = respons, percentile = percentile, standardised = standardised, cache = paste0("data/local") )

workflow <- "rekenen"

```

```{r model-parameters-nat, include=FALSE}
#keuze van parameters
percentile <- "95"
# indicatorname <- paste0("dag_boven_p",percentile)
# indicatorname_inv <- paste0("dag_onder_p",percentile)
standardised <- FALSE
respons <- respons.f("dag_boven_p", percentile = "95")
```


```{r abs-indic-model-without-trend-nat, eval = !file.exists(file.path("data", "local" paste0("modelresultaat_geentrend.rds"))) | workflow == "rekenen"}
dagen_droog_nat <- indic_abs_function_without_trend(modeldata = modeldata, respons = respons, percentile = percentile, standardised = standardised, cache = paste0("data/local") )
```


```{r abs-indic-model-without-trend-uit-cache, include=FALSE}
dagen_droog_nat <- readRDS(file.path("data", "local", paste0("modelresultaat_geentrend.rds" ) ))

for (ma in tijdvenster) {
  respons <- paste0(".", ma, "_", respons.f("dag_onder_p", percentile = "05"))
  namen_intermediaire_tabellen <- readRDS(file.path("cache", paste0("model_namen_intermediair_geentrend",respons, ".rds" ) ))
...
respons <- paste0(".", ma, "_",respons.f("dag_boven_p", percentile = "95"))
namen_intermediaire_tabellen <- readRDS(file.path("cache", paste0("model_namen_intermediair_geentrend",respons, ".rds" ) ))
...
```{r trend-data-without-trend-vc, include=FALSE}

#bewaren resultaat
write_vc(dagen_droog_nat, file.path("data", "result", "dagen_droog_nat"), sorting = c("jaar", "meetpunt", "simulatienr"), strict = FALSE)

```

```{r trend-droog-rubin, message=FALSE, warning=FALSE, include=FALSE, eval = FALSE}

#berekenen van indicator
indic_abs_droog_gem <- dagen_droog_update %>% 
  group_by(jaar) %>% 
...
```{r droogte-nat-indicator-geentrend-met-ci, include=FALSE}
teller <- 1
for (ma in tijdvenster) {
  d <- get(paste0("indic_abs_p05.",ma,"_jaar_stat"))
  d <- d %>% mutate(tijdvenster = ma)  
  if (teller == 1) {
    droogte_basis_05 <- d
  } else {
    droogte_basis_05 <- bind_rows(droogte_basis_05, d)
  }


  d <- get(paste0("indic_abs_p95.",ma,"_jaar_stat"))
  d <- d %>% mutate(tijdvenster = ma)
  if (teller == 1) {
    nat_basis_05 <- d
  } else {
    nat_basis_05 <- bind_rows(nat_basis_05, d)
  }
  
  teller <- teller + 1 
}
droogte_basis_05 <- droogte_basis_05 %>% 
  mutate( gem_jaar = exp(mean),
          og_jaar = exp(p02.5),
          bg_jaar = exp(p97.5)
  )
nat_basis_05 <- nat_basis_05 %>% 
  mutate( gem_jaar = exp(mean),
          og_jaar = exp(p02.5),
          bg_jaar = exp(p97.5)
  )  
...
```{r droogte-indicator, include=FALSE, eval = FALSE}
droogte_ind_j <- dagen_droog_nat %>% 
  group_by(jaar) %>% 
...
```{r nat-indicator, include=FALSE, eval = FALSE}
nat_ind_j <- dagen_droog_nat %>% 
  group_by(jaar) %>% 
  summarise(nat_ind_j = mean(dag_boven_p95)
            ) %>% 
  ungroup()
...
```{r droogte-indicator-met-ci, include=FALSE, eval = FALSE}

droogte_ind_j_s <- dagen_droog_nat %>% 
...
```{r nat-indicator-met-ci, include=FALSE, eval = FALSE}

nat_ind_j_s <- dagen_droog_nat %>% 
  group_by(jaar, simulatienr) %>% 
  summarise(nat_ind_j_s = mean(dag_boven_p95),
            aantal = n(),
            dagen_nat_se = sd(dag_boven_p95)/sqrt(aantal)
            ) %>% 
  ungroup()

nat_ind_j_var <- nat_ind_j_s %>% 
  inner_join(nat_ind_j) %>% 
  group_by(jaar) %>% 
  summarise(dagen_nat_gem_var = mean(dagen_nat_se^2),
            aantalsim = n(),
            imput_var = (1 + 1/aantalsim) * sum((nat_ind_j_s - nat_ind_j)^2/(aantalsim-1)),
            nat_ind_j_var = dagen_nat_gem_var + imput_var
            ) %>% 
  ungroup() %>% 
  dplyr::select (-aantalsim)

nat_ind_j_var <- nat_ind_j_var %>% 
  mutate (rel_bijdrage_imput = round(imput_var/nat_ind_j_var*100,0))


nat_ind_j <- nat_ind_j %>% 
  inner_join(nat_ind_j_var %>% 
               dplyr::select(-starts_with(c("dagen", "imput", "rel")))) %>% 
  mutate(lcl = nat_ind_j - 1.96 * sqrt(nat_ind_j_var),
         ucl = nat_ind_j + 1.96 * sqrt(nat_ind_j_var)
         )

```


```{r droogte-indicator-geentrend-met-ci-vc, include=FALSE}
write_vc(droogte_basis_05, file.path("data", "result", "droogte_indicator"), sorting = c("tijdvenster", "jaar"), root = projroot)
```

```{r nat-indicator-geentrend-met-ci-vc, include=FALSE}
write_vc(nat_basis_05, file.path("data", "result", "nat_indicator"), sorting = c("tijdvenster", "jaar"), root = projroot)
```

Onderstaande tabel geeft de berekende indicator-waarden inclusief het 95% betrouwbaarheidsinterval (CI).

```{r droogte-indicator-ci-tbl, include = TRUE, fig.cap="Droogte-indicator inclusief p05-95-betrouwhaarheidsinterval 1985 - 2022"}
dt <- droogte_basis_05
row.names(dt) <- NULL
dt <-  dt %>% 
    filter(tijdvenster == 0) %>% 
    dplyr::select(contains("jaar")) %>% 
    column_to_rownames("jaar") %>% 
    rename('ondergrens (05% CI)' = og_jaar,
            'bovengrens (95% CI)' = bg_jaar,
            indicator = gem_jaar
            )
...
# code for interactive (html) output
  DT::datatable(dt,
  caption = "Droogte-indicator inclusief p05-95 betrouwhaarheidsinterval (CI) 1985 - 2022"
  ) %>% 
  DT::formatRound(columns = 1:3,
                  digits = 1)
} else {
# code for static (pdf) output
  dt %>% 
  mutate_at(1:3, format, digits = 1, nsmall = 0) %>% 
  kbl(longtable = TRUE,
      booktabs = TRUE,      
      caption = "Droogte-indicator inclusief p05-95 betrouwhaarheidsinterval (CI) 1985 - 2022",
      align = c("l", rep("r",3))
...
De oranje streepjeslijn in de figuur geeft het aantal droge dagen dat jaarlijks gemiddeld verwacht wordt.
...
(ref:droogte-indicator-met-ci-figuur) Droogte-indicator (rode volle lijn) met 95% betrouwbaarheid (grijze zone). De oranje streepjeslijn geeft het aantal droge dagen dat jaarlijks gemiddeld verwacht wordt.

```{r droogte-indicator-met-ci-figuur, fig.cap= "(ref:droogte-indicator-met-ci-figuur)"}
d <- droogte_basis_05 %>% filter(tijdvenster == 0)
g <- ggplot(data = d, aes(x = jaar, y = gem_jaar)) + 
  theme_inbo() +
  geom_ribbon(aes(ymin = og_jaar, ymax = bg_jaar), fill = inbo_lichtgrijs) +
  geom_hline(yintercept = 365 * 0.05, color = inbo_oranje, linetype = "dashed") +
  geom_line(color = inbo_rood, size = 0.5) + 
  labs(y = "aantal droge dagen") + 
  scale_x_continuous(n.breaks = nrow(d)) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, color = "black"))
g
```

```{r nat-indicator-met-ci-figuur, fig.cap= "(ref:nat-indicator-met-ci-figuur)", eval=FALSE}
d <- nat_basis_05 %>% filter(tijdvenster == 0)
g <- ggplot(data = d, aes(x = jaar, y = gem_jaar)) + 
  theme_inbo() +
  geom_ribbon(aes(ymin = og_jaar, ymax = bg_jaar), fill = inbo_lichtgrijs) +
  geom_hline(yintercept = 365 * 0.05, color = inbo_oranje, linetype = "dashed") +
  geom_line(color = inbo_rood, size = 0.5) + 
  labs(y = "aantal natte dagen") + 
  scale_x_continuous(n.breaks = nrow(d)) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, color = "black"))
g
```

```{r verdroging-indicator-met-ci, eval=FALSE}
#x1 - x2 +/- 1.96 * sqrt(sd1^2/n + sd2^2/n)

verdroging_ind_j <- droogte_ind_j %>%
  dplyr::select(-lcl, -ucl) %>% 
  inner_join(nat_ind_j %>% 
               dplyr::select(-lcl, -ucl), by = "jaar") %>% 
  mutate(verdroging_ind_j = droogte_ind_j - nat_ind_j,
         lcl = verdroging_ind_j - 1.96 * sqrt((droogte_ind_j_var + nat_ind_j_var)/2),
         ucl = verdroging_ind_j + 1.96 * sqrt((droogte_ind_j_var + nat_ind_j_var)/2)  
         )
```


```{r verdroging-indicator-met-ci-figuur, fig.cap= "(ref:nat-indicator-met-ci-figuur)", eval=FALSE}
g <- ggplot(data = verdroging_ind_j, aes(x = jaar, y = verdroging_ind_j)) + 
  theme_inbo() +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), fill = inbo_lichtgrijs) +
  geom_line(color = inbo_rood, size = 0.5) + 
  labs(y = "aantal droge dagen - aantal natte dagen") + 
  scale_x_continuous(n.breaks = nrow(nat_ind_j)) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, color = "black"))
g
```
...
Deze trend is te verklaren door de toegenomen meetintensiteit (zie figuur \@ref(fig:watina-metingen-fig).
...
Bij deze indicator is het *verloop* van de waarden nog belangrijker dan de absolute waarden an sich.
...
```{r rollend-gem-droog-met-ci, include=FALSE}
dagen_droog_ma <- dagen_droog_nat %>% 
...
droogte_ind_j_var <- droogte_ind_j_s %>% 
  inner_join(droogte_ind_j_ma, by = join_by(jaar, tijdvenster)) %>% 
  group_by(jaar, tijdvenster) %>% 
  summarise(dagen_droog_gem_var = mean(dagen_droog_se^2),
            aantalsim = n(),
            imput_var = (1 + 1/aantalsim) * sum((droogte_ind_j_s - droogte_ind_j)^2/(aantalsim - 1)),
...
```{r rollend-gem-nat-met-ci, include=FALSE}
dagen_nat_ma <- dagen_droog_nat %>% 
  pivot_longer(cols = starts_with("dag_boven_p95."), names_to = "rollendgem", values_to = "waarde") %>% 
  mutate(tijdvenster = as.integer(str_sub(rollendgem, 14 + 1,-1)),
        rollendgem = "dag_boven_p95.ma"
        ) %>% 
  pivot_wider(names_from = rollendgem, values_from = waarde)

nat_ind_j_ma <- dagen_nat_ma %>% 
  group_by(jaar, tijdvenster) %>% 
  summarise(nat_ind_j = mean(dag_boven_p95.ma)
            ) %>% 
  ungroup()

nat_ind_j_s <- dagen_nat_ma %>% 
  group_by(jaar, simulatienr, tijdvenster) %>% 
  summarise(nat_ind_j_s = mean(dag_boven_p95.ma),
            aantal = n(),
            dagen_nat_se = sd(dag_boven_p95.ma)/sqrt(aantal)
            ) %>% 
  ungroup()

nat_ind_j_var <- nat_ind_j_s %>% 
  inner_join(nat_ind_j_ma, by = join_by(jaar, tijdvenster)) %>% 
  group_by(jaar, tijdvenster) %>% 
  summarise(dagen_nat_gem_var = mean(dagen_nat_se ^ 2),
            aantalsim = n(),
            imput_var = (1 + 1/aantalsim) * sum((nat_ind_j_s - nat_ind_j)^2/(aantalsim - 1)),
...
```{r rollend-gem-verdroging-met-ci, include=FALSE}
verdroging_ind_j_ma <- droogte_ind_j_ma %>%
  dplyr::select(-lcl, -ucl) %>% 
  inner_join(nat_ind_j_ma %>% 
               dplyr::select(-lcl, -ucl), by = c("jaar","tijdvenster")) %>% 
  mutate(verdroging_ind_j = droogte_ind_j - nat_ind_j,
         lcl = verdroging_ind_j - 1.96 * sqrt((droogte_ind_j_var + nat_ind_j_var)/2),
         ucl = verdroging_ind_j + 1.96 * sqrt((droogte_ind_j_var + nat_ind_j_var)/2)  
         )
...
```{r rollend-gem-droog-met-ci-vc, include=FALSE}
write_vc(droogte_ind_j_ma, file.path("data", "result", "droogte_indicator_rollendgem"), sorting = c("jaar", "tijdvenster"), root = projroot)
```

```{r rollend-gem-nat-met-ci-vc, include=FALSE}
write_vc(nat_ind_j_ma, file.path("data", "result", "nat_indicator_rollendgem"), sorting = c("jaar", "tijdvenster"), root = projroot)
```

```{r rollend-gem-verdroging-met-ci-vc, include=FALSE}
write_vc(verdroging_ind_j_ma, file.path("data", "result", "verdroging_indicator_rollendgem"), sorting = c("jaar", "tijdvenster"), root = projroot)
```

Figuur \@ref(fig:rollend-gem-droog-fig) geeft het effect weer van de verschillende tijdvensters.
...
```{r rollend-gem-droog-fig-save, fig.cap="rollend gemiddelde", message=FALSE, warning=FALSE, include=FALSE}
#plot van de trend
...
```{r rollend-gem-droog-fig, fig.cap="De droogte-indicator (cfr. tijdvenster = 0 jaar) en verschillende hiervan afgeleide rollende gemiddelden",  message=FALSE, warning=FALSE}
gplot
```

 
```{r rollend-gem-nat-fig-save, fig.cap="rollend gemiddelde", message=FALSE, warning=FALSE, include=FALSE}
#plot van de trend
gplot <- plotma(nat_ind_j_ma,"nat_ind_j", 18)
png(paste0(file.path("data","result", "figures","dagen-nat-p95"),"-rollendgem",".png"), width = 1500, height = 1500, res = 150)
gplot
dev.off()
```

```{r rollend-gem-nat_fig, fig.cap="De droogte-indicator (cfr. tijdvenster = 0 jaar) en verschillende hiervan afgeleide rollende gemiddelden",  message=FALSE, warning=FALSE}
gplot
```

 
```{r rollend-gem-verdroging-fig-save, fig.cap="rollend gemiddelde", message=FALSE, warning=FALSE, include=FALSE}
#plot van de trend
gplot <- plotma(verdroging_ind_j_ma,"verdroging_ind_j", 0)
png(paste0(file.path("data","result", "figures","dagen-verdroging-p95"),"-rollendgem",".png"), width = 1500, height = 1500, res = 150)
gplot
dev.off()
```

```{r rollend-gem-verdroging_fig, fig.cap="De verdroging-indicator (cfr. tijdvenster = 0 jaar) en verschillende hiervan afgeleide rollende gemiddelden",  message=FALSE, warning=FALSE}
gplot
```
...
```{r abs-indic-model-function-Rubin}
indic_abs_function_Rubin <- function(modeldata, suffix= "", respons, percentile, indicatorname, standardised, cache, tijdvenster) {
  #De instelling van de prior heeft een doorslaggevend effect op de vorm van de trendlijn. 
  #Geven we het model de vrijheid om een trendlijn te bepalen dan voegen we weinig apriori-kennis toe aan het model
  #Voor de random-walk van de tweede orde doen we dat via de prior voor de precisie. Deze bevat twee elementen: de standaarddeviatie en een alfa. Het eerste element geeft aan in welke mate we denken/willen dat de rw2 mag fluctueren en en de alfa de zekerheid die we daarover hebben. Wil je een quasi rechte lijn, dan moeten beide parameters klein ingesteld worden. Laat je ze vrij, dan kan de trend een grillig patroon gaan vertonen.
  ...
  d_jaar <- dagen_droog_nat[dagen_droog_nat$simulatienr == 1,"jaar"]
  lc_iyear <- rbind(
    moving_trend(
  ...
  for (ma in tijdvenster) {
    ...
    saveRDS(c(resultname_stat, resultname_stat_meetpunt, resultname_mov.average, resultname_fitted), 
            file.path(cache, paste0("model_namen_intermediair.",ma,"_",respons, ".rds" )))  
  }
  saveRDS(dagen_droog_nat, file.path(cache, paste0("modelresultaat_", respons, ".rds" )))
...
indic_abs_function <- function(modeldata, suffix= "", respons, percentile, indicatorname, standardised, cache, tijdvenster) {
  ...
  for (ma in tijdvenster) {
    ...
    saveRDS(c(resultname_stat, resultname_stat_meetpunt, resultname_mov.average, resultname_fitted), 
            file.path(cache, paste0("model_namen_intermediair_final.",ma,"_",respons, ".rds" )))  
  }
  saveRDS(dagen_droog_nat, file.path(cache, paste0("modelresultaat_final_", respons, ".rds" )))
...
dagen_droog_bu <- dagen_droog_nat 
```

```{r model-parameters-droog2, include=FALSE}
#keuze van parameters
percentile <- "05"
# indicatorname <- paste0("dag_onder_p",percentile)
# indicatorname_inv <- paste0("dag_boven_p",percentile)
...
```{r abs-indic-model, include=FALSE, eval = !file.exists(file.path("data","local", paste0("modelresultaat_", respons, ".rds" )))}
#bij een eventuele herberekening moeten de velden met de rekenresultaten eerst verwijderd worden
dagen_droog_nat <- dagen_droog_nat %>% 
  dplyr::select(-contains("fitted") )
...
dagen_droog_nat <- indic_abs_function_Rubin(modeldata = modeldata, respons = respons, percentile = percentile, indicatorname = indicatorname, standardised = standardised, cache = paste0("data/local"), tijdvenster = tijdvenster )

```

```{r abs-indic-model-droog-uit-cache, include=FALSE}
dagen_droog_nat <- readRDS(file.path("data", "local", paste0("modelresultaat_", respons, ".rds" ) ))
for (ma in tijdvenster) {
  namen_intermediaire_tabellen <- readRDS(file.path("data", "local", paste0("model_namen_intermediair.",ma,"_",respons, ".rds" ) ))
  for (i in 1:length(namen_intermediaire_tabellen)) {
    assign(namen_intermediaire_tabellen[i], readRDS(paste0(file.path("data", "local", namen_intermediaire_tabellen[i]), ".rds" )))
  }
}
```

```{r model-parameters-nat2, include=FALSE}
#keuze van parameters
percentile <- "95"
indicatorname <- paste0("dag_boven_p",percentile)
standardised <- FALSE
...
```{r abs-indic-model-nat, include=FALSE, eval = !file.exists(file.path("data", "local", paste0("modelresultaat_", respons, ".rds" )))}
#bij een eventuele herberekening moet de velden met de rekenresultaten eerst verwijderd worden
...
```{r abs-indic-model-nat-uit-cache, include=FALSE}
dagen_droog_nat <- readRDS(file.path("data", "local", paste0("modelresultaat_", respons, ".rds" ) ))
for (ma in tijdvenster) {
  namen_intermediaire_tabellen <- readRDS(file.path("cache", paste0("model_namen_intermediair.",ma,"_",respons, ".rds" ) ))
  for (i in 1:length(namen_intermediaire_tabellen)) {
    assign(namen_intermediaire_tabellen[i], readRDS(paste0(file.path("data", "local", namen_intermediaire_tabellen[i]), ".rds" )))
  }
}
...
```{r trend-data-nat-vc, include=FALSE}
...
```{r trend-droog-rubin, message=FALSE, warning=FALSE, include=FALSE}

#berekenen van indicator
indic_abs_droog_gem <- dagen_droog_nat %>% 
  group_by(jaar) %>% 
  summarise_at(vars(starts_with("dag_onder_p05.")), list(gem = mean, med = median)) %>% 
  ungroup
...
```{r trend-nat-rubin, message=FALSE, warning=FALSE, include=FALSE}

#berekenen van indicator
indic_abs_nat_gem <- dagen_droog_nat %>% 
  group_by(jaar) %>% 
  summarise_at(vars(starts_with("dag_boven_p95.")), list(gem = mean, med = median)) %>% 
  ungroup
...
for (ma in tijdvenster) {
  d <- get(paste0("indic_abs_p95.",ma,"_jaar_stat")) %>% 
    ...
    ungroup() %>% 
    mutate(tijdvenster = ma) %>% 
    inner_join(nat_ind_j_ma, by = c("jaar", "tijdvenster"))
  
  if (teller == 1) {
    nat_trend_95 <- d
  } else {
    nat_trend_95 <- bind_rows(nat_trend_95, d)
  }

  d <- get(paste0("indic_abs_p95.",ma,"_mov.average_stat")) %>%
    group_by(jaar, duration) %>%
    summarise(gem_jaar_t = mean(mean) + mean(intercept),
              var_sim = sum(sd^2) / 20 + (1 + 1/20) * sum((mean - mean(mean))^2)/(20 - 1),
              og_jaar_t = gem_jaar_t - 1.96 * sqrt(var_sim),
    ...
    ungroup() %>%
    bind_rows(nat_ind_j_ma %>% filter(tijdvenster == ma)) %>%
    mutate(tijdvenster = ma)

  if (teller == 1) {
    nat_trend.mov_95 <- d
  } else {
    nat_trend.mov_95 <- bind_rows(nat_trend.mov_95, d)
  }
...
Figuur \@ref(fig:trend-fitting-droog-fig) geeft het resultaat van de modelbouw. 
Het model schat o.a. voor elk meetpunt het verwachte aantal droge dagen per jaar en wat de betrouwbaarheid van deze schatting is.
De figuur toont de zowel de werkelijke waarden (licht blauwe punten) als de door het model geschatte gemiddelde waarden (rode lijn) met hun betrouwbaarheidsinterval. 
De verticale rode lijnen geven het bereik van het geschatte verwachte aantal droge dagen per jaar voor de 58 locaties. 


```{r trend-fitting-droog-fig-save, message=FALSE, warning=FALSE, include=FALSE}
#tabel met de indicatorwaarden (rollend gemiddelden) en trend berekening (model) omvormen naar een lange versie
dagen_droog_lang_breed <- dagen_droog_nat %>% 
...
fig <- plotfitting(indic_basis = dagen_droog_lang_breed, respons = "dag_onder_p05.ma", gemid = "p05_mean_fitted", og = p05_p02.5_fitted, bg = p05_p97.5_fitted, yas = "Aantal dagen onder kritische drempelwaarde")
...
(ref:trend-fitting-droog-fig) Het aantal droogte-dagen (lichtblauwe bollen) in vergelijking tot het voor elk meetpunt modelmatig geschatte verwachte aantal droge dagen per jaar (rode lijn) en het betrouwbaarheidsinterval hiervan (grijze zone)

```{r trend-fitting-droog-fig, fig.cap="(ref:trend-fitting-droog-fig)",  message=FALSE, warning=FALSE}
fig
```


```{r trend-fitting-nat-fig-save, message=FALSE, warning=FALSE, include=FALSE}
#tabel met de indicatorwaarden (rollend gemiddelden) en trend berekening (model) omvormen naar een lange versie
dagen_nat_lang_breed <- dagen_droog_nat %>% 
  pivot_longer(cols = starts_with("p95."), names_to = "statistiek", values_to = "waarde") %>% 
  mutate(tijdvenster = as.integer(substr(statistiek, 4 + 1,  str_locate(string = statistiek, pattern = "_") - 1)),
          statistiek = paste0(substr(statistiek,1,3), str_sub(statistiek, start = str_locate(string = statistiek, pattern = "_")[,1], -1)
                                   )
         ) %>% 
...
fig <- plotfitting(indic_basis = dagen_nat_lang_breed, respons = "dag_boven_p95.ma", gemid = "p95_mean_fitted", og = p95_p02.5_fitted, bg = p95_p97.5_fitted, yas = "Aantal dagen boven drempelpeil")
png(paste0(file.path("data","result", "figures","dagen-nat-p95"),"-tijdreeks",".png"), width = 1500, height = 1500, res = 150)
fig
dev.off()

```

```{r trend-fitting-nat-fig, fig.cap="(ref:trend-fitting-nat-fig)",  message=FALSE, warning=FALSE, include=FALSE}
fig
```

Figuur \@ref(fig:trend-droog-fig) geeft de droogte-indicator (tijdvenster = 0 jaar) met zijn betrouwbaarheidsinterval in het rood en de trend met zijn betrouwbaarheidsinterval in het donkerblauw. 
...
```{r trend-droog-fig-save, fig.cap="trend", message=FALSE, warning=FALSE, include=FALSE}
#plot van de trend
gplot <- plottrend(droogte_trend_05,"droogte_ind_j", 18, yrechts = "droogte-indicator")
png(paste0(file.path("data","result", "figures","dagen-droog-p05"),"-trend-allevensters",".png"), width = 1500, height = 1500, res = 150)
gplot
dev.off()
```

```{r trend-droog-fig, fig.cap="(ref:trend-fig)",  message=FALSE, warning=FALSE}
gplot
```
 
```{r trend-nat-fig-save, fig.cap="trend", message=FALSE, warning=FALSE, include=FALSE}
#plot van de trend
gplot <- plottrend(nat_trend_95,"nat_ind_j", 18, yrechts = "nat-indicator")
png(paste0(file.path("data","result", "figures","dagen-nat-p95"),"-trend-allevensters",".png"), width = 1500, height = 1500, res = 150)
gplot
dev.off()
```

```{r trend-nat-fig, fig.cap="(ref:trend-fig)",  message=FALSE, warning=FALSE, include=FALSE}
gplot
```

```{r trend-voor-1-venster-droog-fig-save, fig.cap="trend één venster", message=FALSE, warning=FALSE, include=FALSE}
#plot van de trend
gplot <- plottrend.1venster(droogte_trend_05,"droogte_ind_j", intercept = 18, tijdvenster = 0, yrechts = "droogte-indicator")
png(paste0(file.path("data","result", "figures","dagen-droog-p05"),"-trend-origineel",".png"), width = 1500, height = 1500, res = 150)
gplot
dev.off()
gplot <- plottrend.1venster(droogte_trend_05,"droogte_ind_j", 18, tijdvenster = 6, yrechts = "droogte-indicator")
png(paste0(file.path("data","result", "figures","dagen-droog-p05"),"-trend-6jaar",".png"), width = 1500, height = 1500, res = 150)
gplot
gplot <- plottrend.1venster(droogte_trend_05,"droogte_ind_j", 18, tijdvenster = 8, yrechts = "droogte-indicator")
png(paste0(file.path("data","result", "figures","dagen-droog-p05"),"-trend-8jaar",".png"), width = 1500, height = 1500, res = 150)
gplot
dev.off()
```

```{r trend-voor-1-venster-nat-fig-save, fig.cap="trend één venster", message=FALSE, warning=FALSE, include=FALSE}
#plot van de trend
gplot <- plottrend.1venster(nat_trend_95,"nat_ind_j", intercept = 18, tijdvenster = 0, yrechts = "nat-indicator")
png(paste0(file.path("data","result", "figures","dagen-nat-p95"),"-trend-origineel",".png"), width = 1500, height = 1500, res = 150)
gplot
dev.off()
gplot <- plottrend.1venster(nat_trend_95,"nat_ind_j", 18, tijdvenster = 6, yrechts = "nat-indicator")
png(paste0(file.path("data","result", "figures","dagen-nat-p95"),"-trend-6jaar",".png"), width = 1500, height = 1500, res = 150)
gplot
dev.off()
gplot <- plottrend.1venster(nat_trend_95,"nat_ind_j", 18, tijdvenster = 8, yrechts = "nat-indicator")
png(paste0(file.path("data","result", "figures","dagen-nat-p95"),"-trend-8jaar",".png"), width = 1500, height = 1500, res = 150)
gplot
dev.off()
```


```{r trend-droog-vc, include=FALSE}
#bewaren resultaten
write_vc(droogte_trend_05, file.path("data", "result", "droogte_trend_05"), sorting = c("jaar", "tijdvenster"), root = projroot, strict = FALSE)

dagen_droog_lang_breed <- dagen_droog_lang_breed %>% 
  dplyr::select(-contains(c("boven", "p95")))
write_vc(dagen_droog_lang_breed, file.path("data", "result", "droogte_fitting_05"), sorting = c("meetpunt", "jaar", "simulatienr", "tijdvenster"), root = projroot, strict = FALSE)
```

```{r trend-nat-vc, include=FALSE}
#bewaren resultaten
write_vc(nat_trend_95, file.path("data", "result", "nat_trend_05"), sorting = c("jaar", "tijdvenster"), root = projroot, strict = FALSE)

dagen_nat_lang_breed <- dagen_nat_lang_breed %>% 
  dplyr::select(-contains(c("onder", "p05")))
write_vc(dagen_nat_lang_breed, file.path("data", "result", "nat_fitting_95"), sorting = c("meetpunt", "jaar", "simulatienr", "tijdvenster"), root = projroot, strict = FALSE)
```


```{r trend-droog-csv-voor-nara, include=FALSE}
...
write_csv(droogte_trend_05 %>% 
            ...
            rename(`aantal dagen onder kritisch minimum` = droogte_ind_j,
                   ...
                   bg_jaar_indicator = ucl), file.path("data", "result",
                                                       paste0("figuur_droog_nara_",
                                                              year(Sys.time()),".csv")))
```


```{r trend-nat-csv-voor-nara, include=FALSE}
#bewaren data voor figuur in nara
write_csv(nat_trend_95 %>% 
            dplyr::select(jaar, gem_jaar, og_jaar, bg_jaar, nat_ind_j, lcl, ucl, tijdvenster) %>% 
            rename(`aantal dagen boven hoog drempelpeil` = nat_ind_j,
                   og_jaar_trend =  og_jaar,
                   bg_jaar_trend = bg_jaar,
                   og_jaar_indicator = lcl,
                   bg_jaar_indicator = ucl), file.path("data", "result", 
                                                       paste0("figuur_nat_nara_",
                                                              year(Sys.time()),".csv")))
```

```{r figuur-nara-poging}
df <- droogte_trend_05 %>% 
            dplyr::select(jaar, gem_jaar, og_jaar, bg_jaar, droogte_ind_j, lcl, ucl, tijdvenster) %>% 
  filter(tijdvenster == 0)
g <- ggplot(data = df, aes(x = jaar, y = droogte_ind_j)) + geom_line() +
  geom_ribbon(aes(y = gem_jaar, ymin = og_jaar, ymax = bg_jaar))
g
```
